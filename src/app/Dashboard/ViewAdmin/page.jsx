  ...
  const handleDelete = async (adminData) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/${adminData}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      setViewListAdmin((prevAdminList) =>
        prevAdminList.filter((admin) => admin.id !== adminData)
      );
      ...
      {updateOpen && (
        <div className="fixed inset-0 bg-gray-300 bg-opacity-5 flex flex-col items-center justify-center z-50">
          <form className="flex flex-col bg-white rounded-lg p-5 gap-4 z-50">
            ...
            <p className="text-black bg-yellow-400 font-semibold">
              Update API in progress
            </p>

            <div className="flex gap-4 justify-center">
              {/* <button onClick={(e) => handleUpdate(e, adminUpdate)} className=" opacity-[0.8] rounded-[4px] px-4 py-1 text-white w-fit" disabled={isSubmitting}>{isSubmitting ? 'Updating...' : 'Update'}</button> */}
              <button
      ...
      <div className="overflow-y-auto max-h-screen shadow-lg">
        <table className="w-full table-auto">
          <thead className="font-semibold text-sm">
            <tr>
              <th className="p-8 text-start bg-[#F7F7F7] rounded-ss-lg">
                Name
              </th>
            ...
            {viewListAdmin?.map((data) => (
              <React.Fragment key={data.id}>
                <tr>
                  ...
                  <td>
                    <div className="text-center text-white px-6 py-1.5 rounded-sm bg-[#4BAF4F] w-fit">
                      Active
                    </div>
                  </td>
