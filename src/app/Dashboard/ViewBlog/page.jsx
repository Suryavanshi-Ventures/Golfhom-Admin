  ...
  const listBlog = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/blog?limit=${itemsPerPage}&page=${currentPage}`,
        {
          method: "GET",
          headers: {
  ...
  const handleDeleteBlog = async (blogId) => {
    try {
      ...
      setBlogList((prevBlogList) =>
        prevBlogList.filter((blog) => blog.id !== blogId)
      );
      setDeleteOpen(false);
  ...
  return (
    <ProtectedRoute>
      <div className="flex justify-between">
        ...
        <Link
          href="/Dashboard/ViewBlog/CreateBlog"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {blogList?.map((data) => (
          ...
          >
            ...
            >
              <Image
                src={data.image ?? "/noImageFound.png"}
                alt="Sort"
                width={100}
                height={70}
